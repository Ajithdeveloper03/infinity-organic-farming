...
export default function FarmerProfileScreen() {
  const [activeTab, setActiveTab] = useState<"personal" | "kyc" | "land">(
    "personal",
  );
  return (
      ...
      <ScrollView className="flex-1 pb-24" showsVerticalScrollIndicator={false}>
        ...
        <View className="bg-white mx-6 mt-6 rounded-3xl overflow-hidden shadow-sm border border-gray-100">
          ...
          <View className="items-center px-6 mb-6">
            <Text className="text-gray-900 text-xl font-gotham-bold mb-1">
              Kuppusamy
            </Text>
            <Text className="text-gray-500 font-brandon-medium mb-1">
              Farmer ID: FM10008
            </Text>
          </View>
        </View>
        ...
        <View className="flex-row mx-6 mt-6 bg-gray-200/50 p-1 rounded-2xl">
          ...
          >
            <Text
              className={`font-gotham-bold text-sm ${activeTab === "personal" ? "text-gray-900" : "text-gray-500"}`}
            >
              Personal
            </Text>
          </TouchableOpacity>
          ...
          >
            <Text
              className={`font-gotham-bold text-sm ${activeTab === "kyc" ? "text-gray-900" : "text-gray-500"}`}
            >
              KYC Docs
            </Text>
          </TouchableOpacity>
          ...
          >
            <Text
              className={`font-gotham-bold text-sm ${activeTab === "land" ? "text-gray-900" : "text-gray-500"}`}
            >
              Land Details
            </Text>
          </TouchableOpacity>
            ...
            <View className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
              <View className="bg-blue-50 p-4 rounded-xl mb-6 flex-row items-start border border-blue-100">
                ...
                <Text className="flex-1 text-blue-800 font-brandon text-sm leading-5">
                  Your profile is verified. To update these details, please
                  contact your assigned Field Officer or Support.
                </Text>
              </View>
              <InfoRow
                icon={Phone}
          ...
          {activeTab === "kyc" && (
            <View className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
              <View className="bg-green-50 p-4 rounded-xl mb-6 flex-row items-center border border-green-100">
                ...
                <Text className="flex-1 text-green-800 font-gotham-bold text-sm">
...
const InfoRow = ({ icon: Icon, label, value, isLast = false }: any) => (
  ...
  >
    ...
    <View className="flex-1">
      <Text className="text-gray-400 text-xs font-brandon-medium mb-0.5">
        {label}
      </Text>
