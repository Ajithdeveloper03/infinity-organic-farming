  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <View className="px-6 pt-12 pb-4 bg-white flex-row items-center">
        ...
        <Text className="text-gray-900 text-lg font-gotham-bold">Visit History</Text>
      </View>
      ...
      <ScrollView className="flex-1 pb-24">
        ...
        <View className="px-6 py-4 flex-row justify-between bg-white border-b border-gray-100">
          {['All', 'Upcoming', 'Completed'].map((tab) => (
             ...
             >
               <Text className={`font-gotham-bold ${activeTab === tab ? 'text-white' : 'text-gray-500'}`}>{tab}</Text>
             </TouchableOpacity>
          ))}
        </View>
        ...
        <View className="px-6 pt-6">
          {history.map((visit) => (
            <View key={visit.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 mb-4">
               <View className="flex-row justify-between items-start mb-3">
                 <View className="flex-row items-center">
                   <View className="w-10 h-10 bg-green-100 rounded-full items-center justify-center mr-3">
                     <Text className="text-[#15803d] font-gotham-bold">{visit.initial}</Text>
                   </View>
                   <View>
                     <Text className="text-gray-900 font-gotham-bold">{visit.name}</Text>
                 ...
                 <View className="bg-green-50 px-2 py-1 rounded">
                   <Text className="text-[#15803d] text-[10px] font-gotham-bold">{visit.status}</Text>
                 </View>
               </View>
               ...
               >
                 <Text className="text-gray-500 text-xs font-gotham-bold mr-1">View Report</Text>
